...
public class EnergizedItemManager
{
	 ...
	 * 
	 * @param itemStack
	 *            - ItemStack to discharge
	 * @param amount
	 *            - amount of energy to discharge from the item, usually the
	 *            total amount of energy needed in a TileEntity
	 ...
	 *            - ItemStack to charge
	 * @param amount
	 *            - amount of energy to charge the item with, usually the total
	 *            amount of energy stored in a TileEntity
	...
	public static double charge(ItemStack itemStack, double amount)
	{
		if (itemStack != null)
		{
			if (itemStack.getItem() instanceof IEnergizedItem)
			{
				...
				{
					double energyToSend = Math.min(energizedItem.getMaxTransfer(itemStack), Math
							.min(energizedItem.getMaxEnergy(itemStack) - energizedItem.getEnergy(itemStack), amount));
