...
public class Pos3D extends Vec3d
{
	 ...
	 * 
	 * @param entity
	 *            - entity to create the Pos3D from
	 */
	 ...
	 * 
	 * @param tileEntity
	 *            - TileEntity to create the Pos3D from
	 */
	...
	/**
	 * Creates and returns a Pos3D with values representing the difference
	 * between this and the Pos3D in the parameters.
	 * 
	 * @param vec
	 *            - Vec3 to subtract
	 ...
	 * 
	 * @param dimensionId
	 *            - the dimension this Pos3D is in
	 ...
	 * @param z
	 *            - amount to translate on the z axis
	...
	/**
	 * Performs the same operation as translate(x, y, z), but by a set amount in
	 * a EnumFacing
	 */
	 ...
	 * 
	 * @param pos
	 *            - the Pos3D to find the distance to
	...
	public Pos3D rotate(float yaw, float pitch, float roll)
	{
		...
		xPos = xCoord * Math.cos(yawRadians) * Math.cos(pitchRadians)
				+ zCoord * (Math.cos(yawRadians) * Math.sin(pitchRadians) * Math.sin(rollRadians)
						- Math.sin(yawRadians) * Math.cos(rollRadians))
				+ yCoord * (Math.cos(yawRadians) * Math.sin(pitchRadians) * Math.cos(rollRadians)
						+ Math.sin(yawRadians) * Math.sin(rollRadians));
		zPos = xCoord * Math.sin(yawRadians) * Math.cos(pitchRadians)
				+ zCoord * (Math.sin(yawRadians) * Math.sin(pitchRadians) * Math.sin(rollRadians)
						+ Math.cos(yawRadians) * Math.cos(rollRadians))
				+ yCoord * (Math.sin(yawRadians) * Math.sin(pitchRadians) * Math.cos(rollRadians)
						- Math.cos(yawRadians) * Math.sin(rollRadians));
		yPos = -xCoord * Math.sin(pitchRadians) + zCoord * Math.cos(pitchRadians) * Math.sin(rollRadians)
				+ yCoord * Math.cos(pitchRadians) * Math.cos(rollRadians);
	...
	{
		double x = translation.xCoord * matrix[0] + translation.yCoord * matrix[1] + translation.zCoord * matrix[2]
				+ matrix[3];
		double y = translation.xCoord * matrix[4] + translation.yCoord * matrix[5] + translation.zCoord * matrix[6]
				+ matrix[7];
		double z = translation.xCoord * matrix[8] + translation.yCoord * matrix[9] + translation.zCoord * matrix[10]
				+ matrix[11];
	...
	{
		return obj instanceof Vec3d && ((Vec3d) obj).xCoord == xCoord && ((Vec3d) obj).xCoord == yCoord
